...
class _CodeInterpreterSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="CODE_INTERPRETER_",
        env_file=DOTENV_PATH,
        extra="ignore",
        env_ignore_empty=True
    )
...
class _AzureSearchSettings(BaseSettings, DatasourcePayloadConstructor):
    ...
    def _set_filter_string(self, request: Request) -> str:
        if self.permitted_groups_column:
            ...
            logging.debug(f"USER TOKEN is {
                          'present' if user_token else 'not present'}")
            if not user_token:
                raise ValueError(
...
class _AppSettings(BaseModel):
    ...
    code_interpreter_settings: _CodeInterpreterSettings = _CodeInterpreterSettings()
